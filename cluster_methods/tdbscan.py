...
INPUTS:
    ...
    CEps = Outer radius for density calculation, kilometers
    Eps = Inner radius defining the density calculation, kilometers
...
def getNeighbors(P, CEps, Eps, df, p_index):
    ...
    for index, point in df.iterrows():
        if index > p_index:
            distance = great_circle((center_point['lat'], center_point['lon']), (point['lat'], point['lon'])).kilometers
            if distance < Eps:
